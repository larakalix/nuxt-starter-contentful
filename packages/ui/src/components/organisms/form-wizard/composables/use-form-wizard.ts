...
export function validateWizardContext() {
    const ctx = inject<WizardContext>(FORM_WIZARD_KEY);
    if (!ctx) throw new Error("useWizard() must be used inside <FormWizard>.");

    return ctx as WizardContext;
}

export function createWizardContext(): WizardContext {
    const form = validateFormContext();
    if (!form) throw new Error("Form context is required for FormWizard.");
