...
export const options = {
    ...
    renderNode: {
        ...
        [BLOCKS.HEADING_1]: (node: Block | Inline, next: Next) =>
            `<h1 class="mb-6 text-4xl text-foreground font-semibold leading-tight tracking-tight">${next(
                node.content
            )}</h1>`,
        [BLOCKS.HEADING_2]: (node: Block | Inline, next: Next) =>
            `<h2 class="mb-4 text-2xl text-foreground font-semibold leading-tight tracking-tight">${next(
                node.content
            )}</h2>`,
        [BLOCKS.HEADING_3]: (node: Block | Inline, next: Next) =>
            `<h3 class="mb-3 text-xl text-foreground font-semibold leading-tight tracking-tight">${next(
                node.content
            )}</h3>`,
        [BLOCKS.HEADING_4]: (node: Block | Inline, next: Next) =>
            `<h4 class="mb-2 text-lg text-foreground font-semibold leading-tight tracking-tight">${next(
                node.content
            )}</h4>`,
        [BLOCKS.HR]: () => `<hr class="my-2 md:my-4 border-primary/20" />`,
        [BLOCKS.UL_LIST]: (node: Block | Inline, next: Next) =>
            `<ul class="ml-6 list-disc marker:text-primary">${next(node.content)}</ul>`,
